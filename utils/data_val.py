...
def cv_random_flip(img, label, edge, fix):
    # left right flip
    ...
    return img, label, edge, fix


def randomCrop(image, label, edge, fix):
    border = 30
    ...
    return image.crop(random_region), label.crop(random_region), edge.crop(random_region), fix.crop(random_region)


def randomRotation(image, label, edge, fix):
    mode = Image.BICUBIC
...
class PolypObjDataset(data.Dataset):
    def __init__(self, image_root, gt_root, edge_root, fix_root, trainsize):
    ...
    def __getitem__(self, index):
        ...
        image, gt, edge, fix = cv_random_flip(image, gt, edge, fix)
        image, gt, edge, fix = randomCrop(image, gt, edge, fix)
        image, gt, edge, fix = randomRotation(image, gt, edge, fix)
        ...
        return image, gt, edge_small, fix
...
def get_loader(image_root, gt_root, edge_root, fix_root, batchsize, trainsize, shuffle=True, num_workers=12, pin_memory=True):
    dataset = PolypObjDataset(image_root, gt_root, edge_root, fix_root, trainsize)
...
if __name__ =='__main__':
    train_root = '/data0/hcm/dataset/COD/TrainDataset/'
    batchsize = 36
    trainsize = 512
